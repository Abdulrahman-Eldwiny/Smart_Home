...
# 358 "./mcc_generated_files/examples/../pin_manager.h"
void PIN_MANAGER_Initialize (void);
# 370 "./mcc_generated_files/examples/../pin_manager.h"
...
uint8_t pass_entry_counter=0;
RealTimeClockDS1307_T RealTimeClockDS1307;
uint8_t Full_Password[4]={0xFF,0xFF,0xFF,0xFF};
uint8_t Password_Check[4]={0xFF,0xFF,0xFF,0xFF};
...
void main(void)
{
    ...
    (INTCONbits.PEIE = 1);
        ...
        if(ON_counter == 0){
            ...
            {
                pressed_button = 0xFF;
                while (pressed_button == 0xFF)
                {
                    pressed_button = keypad_get_value();
                }
                Full_Password[pass_counter]= pressed_button;
                LCD_vSend_char(pressed_button);
                _delay((unsigned long)(((uint16_t)300)*(16000000/4000.0)));
                LCD_movecursor(2,12+pass_counter);
                LCD_vSend_char((uint8_t)'*');
  ...
  LCD_clearscreen();
        ...
        while(Password_Approval==0)
        {
            ...
            while (pass_counter<4)
            {
                ...
                Password_Check[pass_counter]= pressed_button;
                LCD_vSend_char(pressed_button);
                _delay((unsigned long)(((uint16_t)300)*(16000000/4000.0)));
                LCD_movecursor(2,12+pass_counter);
                LCD_vSend_char((uint8_t)'*');
            ...
            if(pressed_button == '1'){
                ...
                if(pressed_button == '1')
                {
                    ...
                    if(pressed_button == '1'){
                        ...
                        LCD_clearscreen();
                        if(conditioner_state==1){
                        LCD_vSend_string("state:ON");
                        LCD_movecursor(2,1);
                        LCD_vSend_string("Temp: ");
                        LCD_vSend_char(DATAEE_ReadByte(0x10)+(uint8_t)'0');
                        LCD_vSend_char(DATAEE_ReadByte(0x11)+(uint8_t)'0');
                        }
                        else{
                    ...
                    else if(pressed_button == '3')
                    {
                        ...
                        if (pressed_button <'0' || pressed_button >'9' )
                        {
                            LCD_clearscreen();
                            LCD_vSend_string("Wrong input");
                            _delay((unsigned long)((500)*(16000000/4000.0)));
                            ...
                            LCD_vSend_char(pressed_button);
                            Input_temp = pressed_button-(uint8_t)'0';
                            pressed_button = 0xFF;;
